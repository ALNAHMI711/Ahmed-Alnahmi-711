...
def test_scans_zip_members():
 blob=BytesIO()
 with ZipFile(blob,'w') as archive: archive.writestr('strategy.py','os.system("whoami")')
...
def test_rejects_zip_path_traversal():
 blob=BytesIO()
 with ZipFile(blob,'w') as archive: archive.writestr('../escape.py','class Strategy: pass')
 assert inspect_upload('strategy.zip',blob.getvalue())['status']=='REJECTED'
