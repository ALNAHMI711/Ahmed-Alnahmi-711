...
${section('signals', 'الإشارات', `${unavailable('لا توجد إشارات حاليًا. API الإشارات المتاح يحلل رسالة مرسلة فقط ولا يعيد قائمة إشارات محفوظة.')}`, 'SIGNALS')}
...
function bindTerminal() {
  ...
  document.querySelector<HTMLFormElement>('#account-form')!.addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const output = document.querySelector('#account-status')!; try { const result = await request<Notice>('/api-accounts', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(form))) }); form.reset(); output.textContent = result.message ?? 'تم الحفظ.'; void loadAccounts(); } catch (error) { output.textContent = error instanceof Error ? error.message : 'تعذر حفظ الحساب'; } });
  document.querySelector<HTMLFormElement>('#strategy-form')!.addEventListener('submit', async event => { event.preventDefault(); const body = new FormData(event.currentTarget); const output = document.querySelector('#scan-result')!; try { const result = await request<StrategyScan>('/strategies/scan', { method: 'POST', body }); output.textContent = JSON.stringify(result, null, 2); } catch (error) { output.textContent = error instanceof Error ? error.message : 'تعذر فحص الملف'; } });
}
