...
class Signal:
    symbol:str; market:str; side:str; entries:tuple[float,...]; take_profits:tuple[float,...]; stop_loss:float
def parse(message: str) -> Signal:
    ...
    if not (symbol_match and market_match and side_match and entries and tps and sl): raise ValueError("الإشارة غير مكتملة أو بصيغة غير مدعومة")
    if len(tps)>7: raise ValueError("الحد الأقصى سبعة أهداف ربح")
