...
def bootstrap_admin(db) -> None:
    if db.scalar(select(User).limit(1)) is None and settings.first_run_password:
        db.add(User(username=settings.admin_username, password_hash=hasher.hash(settings.first_run_password), role="admin"))
        db.commit()
...
def login(username: str, password: str, response: Response) -> dict:
    with SessionLocal() as db:
        ...
        db.add(DbSession(user_id=user.id, token_hash=digest(raw), expires_at=datetime.now(timezone.utc) + timedelta(minutes=settings.session_timeout_minutes)))
        db.commit()
...
def admin(user: CurrentUser) -> User:
    if user.role != "admin":
        raise HTTPException(403, "صلاحية administrator مطلوبة")
    return user
