...
class WebSocketDispatcher:
    ...
    async def connect(self, socket: WebSocket) -> Connection | None:
        ...
        try: user = self._session_user(session or "")
    ...
    async def dispatch(self, message: object, connection: Connection) -> dict | None:
        ...
        try: connection.user = self._session_user(connection.session)
        except HTTPException:
            await self.disconnect(connection, 1008, "session expired or revoked")
            return None
        if not isinstance(message, dict) or not isinstance(message.get("action"), str): return {"type":"errors", "code":"invalid_message"}
    ...
    async def publish(self, event_type: str, payload: dict, *, user_id: str | None = None) -> None:
        ...
        for connection in tuple(self._connections):
            try: user = self._session_user(connection.session)
            except HTTPException:
                await self.disconnect(connection, 1008, "session expired or revoked")
                continue
            connection.user = user
            if event_type not in connection.subscriptions: continue
            # Account ownership is not modelled yet; never leak account events to
            # non-admin connections unless the publisher provides the owning user.
            if user.role != "admin" and (user_id is None or user.id != user_id): continue
    ...
    async def _ping(self, _: dict, __: Connection) -> dict: return {"type":"pong"}
    async def _subscribe(self, message: dict, connection: Connection) -> dict:
        topics = message.get("topics", [message.get("topic")])
        if not isinstance(topics, list) or not topics or any(topic not in EVENT_TYPES for topic in topics): return {"type":"errors", "code":"invalid_subscription"}
