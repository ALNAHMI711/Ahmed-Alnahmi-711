...
class BinanceAdapter(ExchangeAdapter):
    ...
    def __init__(self, market: Market, api_key: str, api_secret: str, client: httpx.AsyncClient | None = None):
        ...
        self.base_url = (
            ...
            else ("https://dapi.binance.com" if market == Market.COIN_M else "https://[unknown]")
        )
    ...
    async def validate_symbol(self, symbol: str) -> bool:
        response = await self.client.get(self.base_url + ("/api/v3/exchangeInfo" if self.market == Market.SPOT else "/fapi/v1/exchangeInfo"))
        response.raise_for_status()
        return any(item["symbol"] == symbol for item in response.json().get("symbols", []))
    ...
    async def fills(self, symbol: str, start_time: int | None = None) -> list[ExchangeFill]:
        ...
        params = {k: v for k, v in {"symbol": symbol, "startTime": start_time}.items() if v is not None}
        rows = await self._signed_get("/api/v3/myTrades" if spot else "/fapi/v1/userTrades", params)

        result: list[ExchangeFill] = []
        for row in rows:
            trade_id = str(row.get("id"))
            order_id = str(row.get("orderId")) if row.get("orderId") is not None else None
            symbol_r = row.get("symbol")
            # side: spot uses 'isBuyer', futures may use 'buyer'
            is_buyer = row.get("isBuyer") if spot else row.get("buyer")
            side = "BUY" if is_buyer else "SELL"
            # quantity/price/quote
            qty = row.get("qty") or row.get("quantity") or row.get("executedQty") or row.get("origQty")
            price = row.get("price") or row.get("p") or row.get("avgPrice")
            quote_qty = row.get("quoteQty") or row.get("cumQuote") or row.get("quoteQuantity") or "0"
            fee = row.get("commission") or row.get("fee") or "0"
            fee_asset = row.get("commissionAsset") or row.get("feeAsset")
            time_val = row.get("time") or row.get("transactTime") or row.get("updateTime")
            occurred_at = _time(time_val) if time_val is not None else datetime.utcnow()
    ...
    async def funding(self, symbol: str, start_time: int | None = None) -> list[FundingPayment]:
        if self.market in (Market.SPOT, Market.CROSS_MARGIN, Market.ISOLATED_MARGIN):
            return []
        params = {k: v for k, v in {"symbol": symbol, "incomeType": "FUNDING_FEE", "startTime": start_time}.items() if v is not None}
        rows = await self._signed_get("/dapi/v1/income" if self.market == Market.COIN_M else "/fapi/v1/income", params)
    ...
    async def mark_price(self, symbol: str) -> Decimal:
        path = (
            "/api/v3/ticker/price"
            if self.market in (Market.SPOT, Market.CROSS_MARGIN, Market.ISOLATED_MARGIN)
            else ("/dapi/v1/premiumIndex" if self.market == Market.COIN_M else "/fapi/v1/premiumIndex")
        )
