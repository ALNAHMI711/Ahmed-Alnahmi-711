...
def inspect_upload(filename: str, content: bytes) -> dict:
    if Path(filename).name != filename:
        return {"status": "REJECTED", "reason": "اسم الملف غير آمن"}
    suffix = Path(filename).suffix.lower()
    if suffix not in ALLOWED:
        return {"status": "REJECTED", "reason": "امتداد الملف غير مسموح"}
    if len(content) > MAX_UPLOAD:
        return {"status": "REJECTED", "reason": "حجم الملف يتجاوز الحد"}
    ...
    if suffix == ".zip":
        try:
            with ZipFile(BytesIO(content)) as archive:
                ...
                if len(files) > MAX_MEMBERS or total > MAX_UNPACKED:
                    return {"status": "REJECTED", "reason": "محتوى ZIP كبير جدًا"}
                for item in files:
                    if item.flag_bits & 1 or _unsafe_member(item.filename) or item.is_dir():
                        if item.flag_bits & 1 or _unsafe_member(item.filename):
                            return {"status": "REJECTED", "reason": "محتوى ZIP غير آمن"}
                        continue
                    if (item.external_attr >> 16) & 0o170000 == 0o120000 or item.file_size > MAX_MEMBER or (item.compress_size and item.file_size / item.compress_size > 100):
                        return {"status": "REJECTED", "reason": "محتوى ZIP غير آمن أو كبير"}
                    findings.extend(_findings(archive.read(item)))
        except BadZipFile:
            return {"status": "REJECTED", "reason": "ملف ZIP غير صالح"}
