...
def validate_exit(kind: str, position_side: str, trigger_price: Decimal, mark_price: Decimal, quantity: Decimal) -> None:
    ...
    if kind not in {"TP", "SL", "TRAILING"}: raise ValueError("unsupported exit kind")
    if position_side not in {"LONG", "SHORT"}: raise ValueError("position_side must be LONG or SHORT")
    if trigger_price <= 0 or mark_price <= 0 or quantity <= 0: raise ValueError("exit price and quantity must be positive")
    if kind == "TP": valid = trigger_price > mark_price if position_side == "LONG" else trigger_price < mark_price
    elif kind == "SL": valid = trigger_price < mark_price if position_side == "LONG" else trigger_price > mark_price
    else: valid = True
    if not valid: raise ValueError("exit trigger is on the unsafe side of the current mark")

def exit_order_request(kind: str, position_side: str, quantity: Decimal, trigger_price: Decimal, client_request_id: str) -> dict:
    kind, position_side = kind.upper(), position_side.upper()
    if kind not in {"TP", "SL"} or position_side not in {"LONG", "SHORT"}: raise ValueError("unsupported TP/SL request")
    if quantity <= 0 or trigger_price <= 0 or not client_request_id: raise ValueError("invalid TP/SL request")
...
def binance_exit_order(kind: str, position_side: str, market: Market, quantity: Decimal, trigger_price: Decimal, client_request_id: str) -> dict:
    ...
    kind = kind.upper(); position_side = position_side.upper()
    if market in (Market.SPOT, Market.CROSS_MARGIN, Market.ISOLATED_MARGIN):
        # STOP/TAKE_PROFIT_LIMIT are price-protected limit exits; caller must supply a limit price.
    ...
    if market in (Market.USDS_M, Market.COIN_M):
        # Futures conditional exits are native STOP_MARKET / TAKE_PROFIT_MARKET and reduce-only.
