...
async def portfolio(user:CurrentUser,account_id:str|None=None):
...
async def positions(user:CurrentUser,account_id:str|None=None,symbol:str|None=None):
...
async def orders(user:CurrentUser,account_id:str|None=None,symbol:str|None=None):
    try:return await _exchange_data(_selected_account(user,account_id),'orders',symbol)
    except HTTPException as error:return {'status':'unavailable','reason':error.detail,'data':[]}
@app.get('/api/v1/market-data')
async def market_data(user:CurrentUser,account_id:str|None=None,symbol:str|None=None):
